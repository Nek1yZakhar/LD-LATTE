...
> 🌐 **Public Submission Hub & Demo UI**: Интерактивный веб-интерфейс проекта доступен в папке [`app/`](file:///c:/Users/Admin/Desktop/LD%20LATTE/app) (`cd app && npm run dev`). Быстрый навигатор для проверяющего эксперта доступен в [`docs/START.md`](file:///c:/Users/Admin/Desktop/LD%20LATTE/docs/START.md).
> 🚀 **Public Deployment URL**: `[Pending TICKET-10G Public Vercel Deployment]`
...
* **Fallback**: [Playwright](https://playwright.dev) с авторизованной сессией (`sessionid`) для имитации действий пользователя в браузере при возникновении блокировок.
* **Emergency Only**: [Apify](https://apify.com) — облачный SaaS-парсер, используемый исключительно как неактивированный резерв. **В данном прогоне Apify НЕ использовался** (сбор на 100% выполнен собственным парсером).
...
* Ручное решение капч (manual captcha solving) допускается на уровне MVP, так как объемы сбора данных ограничены масштабами целевого пула.
...
* **Primary**: [Qwen3-Embedding-0.6B](https://huggingface.co/Qwen/Qwen3-Embedding-0.6B) — легкая и эффективная мультиязычная модель эмбеддингов (локальный CUDA/CPU инференс, $0.00).
...
### Reranker (Реранкер)
* **Primary**: [BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) — мультиязычный кросс-энкодер для семантического ранжирования кандидатов (локально, $0.00).
* **Fallback**: Jina Multilingual Reranker.

### LLM Stack (Языковые модели)
* **Primary Provider**: **DeepSeek-V4 (OpenRouter API)** / **Groq API**.
* **Разделение ролей**:
  * Быстрые модели извлекают структурированные факты из био и постов.
  * DeepSeek-V4 пишет персонализированные предложения (barter-offers) с Anti-Robotic QA фильтром против канцелярита ($0.01 за прогон).

### Visual Layer (Мультимодальный визуальный анализ)
* **Live OpenRouter API**: **Qwen2.5-VL-72B** (`qwen/qwen-2.5-vl-72b-instruct`).
* **Живой прогон**: Проведен подлинный мультимодальный прогон через API с передачей фото аватарок Top-5 кандидатов ($0.003 за весь шорт-лист).
* **Важное ограничение**: VLM **не участвует** в первичном поиске и фильтрации. Он используется строго на этапе **sanity-check для финального шорт-листа (top 3–5 кандидатов)** для оценки эстетического соответствия аватарки и стиля блогера палитре LD Latte.
...
### Шаг 0: Быстрый запуск веб-сайта Demo UI (Рекомендуется для проверки)
Для быстрой визуальной инспекции работы пайплайна, промптов и офферов запустите веб-интерфейс:
```bash
# 1. Перейдите в каталог приложения
cd app

# 2. Установите npm-зависимости (при первом запуске)
npm install

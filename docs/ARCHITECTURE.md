...
* **`src/analyzers`**: Синтезирует детальный профиль идеального инфлюенсера на основе объединенных данных успешных интеграций (seed). Модуль `src/analyzers/portrait.py` реализован в TICKET-05.
* **`src/search`**: Модуль поиска кандидатов. Сканирует и нормализует пул кандидатов, применяет правило-ориентированные фильтры (rule-based filters) и подготавливает профили `CandidateProfile` для скоринга. Модуль `src/search/discover.py` реализован в TICKET-06.
* **`src/scoring`**: Модуль многоуровневого ранжирования. Отвечает за расчет эмбеддингов (`src/scoring/embed.py`), пофичевый скоринг (`src/scoring/score.py`), реранкинг кросс-энкодером `BAAI/bge-reranker-v2-m3` с сигмоидной нормализацией (`src/scoring/rerank.py`, CLI: `python -m src.scoring.rerank`) и финальную визуальную валидацию через VLM Qwen2.5-VL / Qwen3-VL с поддержкой mock sandbox mode (`src/scoring/vlm_sanity.py`, CLI: `python -m src.scoring.vlm_sanity`).
...
* **`src/shared`**: Общие утилиты, Pydantic-контракты данных (`src/shared/models.py`), обертки над API Groq/OpenRouter, функции логирования и загрузки `.env`.
...
3. **Этап 3: Hand-crafted Feature Scoring (Фичевый скоринг)**:
   ...
   $$\text{score}_{\text{features}} = w_1 \cdot \text{niche\_match} + w_2 \cdot \text{er\_score} + w_3 \cdot \text{sponsorship\_score} + w_4 \cdot \text{recency\_score} + w_5 \cdot \text{language\_score}$$
   *Реализовано в `src/scoring/score.py`. Результат агрегируется с `semantic_similarity` в `composite_score` = $0.5 \cdot \text{semantic\_similarity} + 0.5 \cdot \text{score}_{\text{features}}$. Поле `cross_encoder_score` устанавливается в 0.0 до выполнения TICKET-08.*
4. **Этап 4: Cross-Encoder Reranking (Пересортировка)**:
   Сравнение текста био, ниши и последних постов кандидата с текстовым описанием идеального портрета через **BAAI/bge-reranker-v2-m3** (`src/scoring/rerank.py`). Оценки-логиты нормализуются сигмоидной функцией $\text{score}_{\text{norm}} = \frac{1}{1 + e^{-\text{logit}}}$ в диапазон $[0.0, 1.0]$. Композитный балл обновляется по формуле:
   $$\text{composite\_score} = 0.35 \cdot \text{semantic\_similarity} + 0.35 \cdot \text{features\_score} + 0.30 \cdot \text{cross\_encoder\_score\_norm}$$
   Полные результаты экспортируются в `data/processed/candidates_reranked.json`, а топ-10 кандидатов сохраняются в `data/processed/shortlist_raw.json` без модификации исходного `data/processed/candidates_scored.json`.
5. **Этап 5: VLM Visual Sanity Pass (Визуальный контроль)**:
   ...
   * Вызывается **только для топ 3–5 кандидатов** из `data/processed/shortlist_raw.json` (`src/scoring/vlm_sanity.py`).
   * VLM (**Qwen2.5-VL-72B via OpenRouter API**) вызвана в реальном прогоне мультимодального API с передачей base64 фото аватарок финалистов. Она проверяет соответствие эстетике бренда (минимализм, пастельная гамма, отсутствие визуального шума, качество кадров), формирует вердикт `vlm_sanity_passed`, заметки `vlm_aesthetic_notes` и список `grounding_facts`. Стоимость реального визуального аудита 5 финалистов составила **~$0.003**. Итоговый шорт-лист сохранен в `data/processed/shortlist_final.json` по Pydantic-контракту `FinalShortlistEntry`.
...
2. **Fallback path (Playwright authenticated session + SOCKS5 Proxy)**:
   При возникновении ошибок или капч Instaloader-а запускается локальный браузер через Playwright. Он использует сохраненную сессию авторизованного технического аккаунта (`data/instagram_session.json` с `sessionid` и `ds_user_id`) и SOCKS5 прокси (`socks5://127.0.0.1:2080`).
   *Примечание о 100% реальных данных*: Из 34 исходных ссылок в `Блогеры - Лист1.csv` 19 аккаунтов являются реально действующими и спарсены в `data/processed/seed_enriched.json`. Оставшиеся 15 ссылок откликиваются HTTP 404/400 на серверах Instagram (удалены/переименованы) и строго пропускаются без создания фейковых данных в соответствии со строгой политикой ([`docs/POLICY.md`](file:///c:/Users/Admin/Desktop/LD%20LATTE/docs/POLICY.md)).

3. **Emergency Only path (Apify SaaS)**:
   Облачные прокси-парсеры. Используются исключительно как неактивированный резерв. **В данном прогоне Apify НЕ использовался** (сбор на 100% выполнен собственным парсером Instaloader + Playwright).

### Защита от банов и политика качества данных (Strict Zero-Mock Policy):
* **Strict Zero-Mock Data Policy**: Категорический запрет на генерацию фейковых профилей или выдуманных полей в боевом скрапере.
* Внедрение случайных пауз (sleep delay) между сетевыми запросами.
* Локальное кэширование ответов (snapshots): если данные профиля уже были успешно собраны за последние 24 часа (`CACHE_TTL_HOURS=24`), повторный запрос к сети не производится (`data/processed/seed_enriched.json`).
...
| **Portrait Synthesizer (Синтез портрета)** | Llama-3-70b (Groq) | Gemma-4-31B / Gemma-4-26B (OpenRouter free) | Требуются хорошие аналитические способности для обобщения данных. Реализовано в `src/analyzers/portrait.py`. |

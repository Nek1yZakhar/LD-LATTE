...
5. **Этап 5: VLM Visual Sanity Pass (Визуальный контроль)**:
   ...
   * VLM (Qwen2.5-VL / Qwen3-VL) проверяет соответствие эстетике бренда (минимализм, пастельная гамма, отсутствие визульного шума, качество кадров), поддерживает mock sandbox mode для офлайн-тестирования, формирует вердикт `vlm_sanity_passed`, заметки `vlm_aesthetic_notes` и список `grounding_facts`. Итоговый шорт-лист сохраняется в `data/processed/shortlist_final.json` по Pydantic-контракту `FinalShortlistEntry`.
